...
vector<TimeAndPrice> ComputeTimePrice(Person &p)
{
	...
	for(int i = 0; i < size; i++)
	{
		...
		total_price += price_per_minute[hours1] * (60 - minutes1) + price_per_minute[hours2] * minutes2;
		for(int i = hours1 + 1; i % 24 < hours2; i++)
		{
			total_price += price_per_hours[i];
		}
...
void PairDate(Person &p)
{
	...
	{
		if(v[i].tag == "off_line" && v[i - 1].tag == "on_line")
		{
			p.start_time.push_back(v[i]);
...
void GenerateBills(map<string, Person> mp, int price_per_minute[], string mon)
{
	...
	map<string, Person>::iterator it = mp.end();
	for(it--; it != mp.begin(); it--)
	{
		...
		for(int j = 0; j < size; j++)
		{
			...
			cout << setprecision(2) << price[j].price << endl;
			total_price += price[j].price;
		}
		cout << setprecision(6);
		cout << "Total amount: $";
		cout << setiosflags(ios::fixed);
		cout << setprecision(2) << total_price << endl;
		cout << setprecision(6);
	}
}
